...
async function collectFormData() {
  ...
  return {
    ...
    erf: document.getElementById('erf-size')?.value || '',
    gla: document.getElementById('gla')?.value || '',
...
// =====================
// Word Summary (fixed anchor DOM append)
// =====================
async function downloadWordDoc() {
  ...
  const erf = document.getElementById("erf-size")?.value?.trim();
  const gla = document.getElementById("gla")?.value?.trim();
